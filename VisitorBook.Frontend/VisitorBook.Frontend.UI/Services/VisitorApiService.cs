...
namespace VisitorBook.Frontend.UI.Services
{
    public class VisitorApiService
    {
        ...
        public async Task<PagedTableListOutput<VisitorOutput>> GetTableData(DataTablesOptions dataTablesOptions)
        {
            var response = await _httpClient.PostAsJsonAsync("visitors/gettabledata", dataTablesOptions);

            var result = await response.Content.ReadFromJsonAsync<PagedTableListOutput<VisitorOutput>>();
