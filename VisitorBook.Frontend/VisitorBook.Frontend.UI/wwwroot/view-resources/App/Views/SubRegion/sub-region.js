...
$(document).ready(function () {
    ...
    var editModalTitleText = document.getElementById('SubRegionsEditModalTitleText').value
...
function loadDataTable(
    ...
    dataTable = $('#tblData').DataTable({
        ...
        responsive: true,
        ajax: {
            url: '/app/subregion/getall',
            type: "POST",
            datatype: "json"
        },
        columns: [
            { data: 'name', width: '40%' },
            { data: 'region.name', width: '40%' },
            {
                data: 'id',
                render: function (data) {
                    return `
                            <div class="d-flex justify-content-around align-items-center">
                               <a onclick="showInPopup('/app/subregion/edit/${data}',
                               '${editModalTitleText}')" class="btn btn-warning"> ${editBtnText}</a>
                               <a onclick=deleteRecord('/app/subregion/delete/${data}') class="btn btn-danger">
                                 ${deleteBtnText}
                               </a>
                            </div>
                            `
